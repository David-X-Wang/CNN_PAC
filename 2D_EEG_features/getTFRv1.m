...
fb = cwtfilterbank('Signallength',450,'wavelet','Morse','VoicesPerOctave',10,'SamplingFrequency',250,'FrequencyLimits',[1 100]);
fset= [45:90];
t = linspace(1,1.8,900);
...
for subind = 1:length(subj_sessions)
    ...
    for Eleind = 1:length(elecList{subind,rind})
      ...
      nencEEG1 = (nencEEG1'./std(nencEEG1'))';
        ...
        for k = 1:length(encEvents)
            ...
            [wt,f] = cwt(encEEG1(k,:),'FilterBank',fb);
            QPC_recalled(acc1,:,:)= zscore(abs(wt));
            labelRe(acc1) = 1;             
            acc1=acc1+1;        
        end

        for k = 1:min(1000,length(nencEvents))
            if isnan(nencEEG1(k,:))
                continue
            end
            [wt,f] = cwt(nencEEG1(k,:),'FilterBank',fb);
            QPC_nonrecalled(acc2,:,:)= zscore(abs(wt));         
            labelNR(acc2) = 0;
            acc2=acc2+1;          
        end
    ...
    save([RegionList{rind},'-TFRv1.mat'],'Thisfeature','-v7.3')
    save([RegionList{rind},'-TFRlabelsv1.mat'],'labels')
